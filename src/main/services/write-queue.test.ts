...
describe("write-queue: no overlapping writes (property)", () => {
  it("writes never overlap for any schedule of pushes", async () => {
    await assert(
      asyncProperty(
        array(integer({ min: 0, max: 50 }), { minLength: 1, maxLength: 20 }),
        async (values) => {
          const intervals: Array<[number, number]> = [];
          let time = 0;
          const q = createWriteQueue<number>(async () => {
            const start = time++;
            // Simulate some async work (resolved microtask).
            await Promise.resolve();
            const end = time++;
          ...
          // Check no two intervals overlap: [a,b] and [c,d] overlap if a<d && c<b.
